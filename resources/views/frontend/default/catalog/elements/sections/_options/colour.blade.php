@if(count($product_colours)>0)
    <div class="hidden-first">
        <div class="float-left mr-4 mt-1" style="{{ $isPhoneDevice ? 'width: 12%;' : 'width: 20%;' }}">Color:</div>
        <el-select style="float: left;" v-model="selectedColour" placeholder="Please Choose Colour ...">
            <el-option
                    v-for="(item, colorIndex) in productColours"
                    :key="item.name"
            ...
            >
                ...
                <div class="color-image-box" v-if="item.type=={{ \App\Models\Utils\ColourTool::$TYPE_IMAGE }}">
                    <span style="float: left">@{{ item.name }}  @{{ item.extra_money>0 ? ' + $'+item.extra_money : null }}</span>
                    <img :src="'/storage/'+item.imageUrl" style="width:33px;height:33px;float:right;margin-bottom:10px;" />
                </div>
            </el-option>
        </el-select>

        <div class="selected-color-display" v-if="selectedColour" style="{{ $isPhoneDevice ? 'float:left;margin-left:0px;' : null }}">
            ...
            <div class="color-image-box" v-if="selectedColour.type=={{ \App\Models\Utils\ColourTool::$TYPE_IMAGE }}">
                <img class="thumb" :src="'/storage/'+selectedColour.imageUrl"  style="{{ $isPhoneDevice ? 'margin-top:20px;width:190px;height:auto;' : null }}" />
            </div>
        </div>
